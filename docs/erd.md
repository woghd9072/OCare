...
> 이 문서의 테이블 정의는 실제 생성된 스키마(`information_schema`)에서 추출했습니다.
> 설명란은 마이그레이션 파일에 작성한 컬럼 COMMENT 를 그대로 옮긴 것이라 DB 와 항상 일치합니다.

## 관계도

![OCare ERD](images/erd.svg)

> 다이어그램은 `docs/images/erd.svg` 이며, 스키마가 바뀌면 함께 갱신합니다.
> 전체 컬럼과 설명은 아래 [테이블 정의](#테이블-정의)에 있습니다.
...
새 recordkey 가 발급되는데 과거 데이터는 이전 키에 남아 있어야 합니다.
실제 입력 데이터도 recordkey 4개가 각각 다른 출처(삼성 2 / 애플 2)로 들어옵니다.

`record_key` 에 전역 UNIQUE 를 두어 같은 키가 두 회원에 매핑되는 것을 막습니다.
같은 키가 두 회원에 붙으면 수집 데이터의 주인이 모호해집니다.
...
- 수신/저장/중복 건수가 남아, 나중에 데이터가 비어 보일 때
  **단말이 안 보낸 것인지 서버가 거른 것인지** 구분할 수 있습니다.

### 3. `health_records` 는 append-only

한 번 저장하면 수정하지 않으므로 `updated_at` 을 두지 않았습니다.
수정 시각 컬럼이 있으면 "언젠가 수정될 수 있다"는 잘못된 신호를 줍니다.
...
두 출처의 시각 표기가 달라 이 구분이 필수입니다. 삼성은 오프셋 없는 로컬 시각,
애플은 ISO8601 UTC 로 보냅니다. 서버는 모두 UTC 로 모으고, 집계는 KST 날짜로 수행합니다.
...
애플의 걸음수는 `688.5509846105425` 같은 소수입니다.
부동소수로 누적하면 원본 합계와 어긋나므로 저장·집계를 DECIMAL 로 수행합니다.

일별 집계의 `steps` 만 `INT` 인데, 과제 명세가 `steps int` 이기 때문입니다.
**소수 걸음을 모두 더한 뒤 마지막에 한 번만 반올림**합니다.
...
`dedup_key` 에 **측정값(걸음수 등)은 넣지 않습니다.** 단말이 같은 구간의 값을 보정해
다시 보낼 수 있는데, 값을 키에 포함하면 보정 전후가 서로 다른 측정으로 취급되어 중복 저장됩니다.
...
`health_records` 에는 `record_key` 외래키를 걸지 않았습니다.
1회 수집에 최대 1,497건이 배치 INSERT 되는데, 행마다 부모 조회·잠금이 발생하면
수집 성능이 떨어집니다. 대신 `upload_id` 외래키로 무결성 체인이 이어져 있어
`health_records → health_uploads → record_keys → members` 로 소유 관계가 보장됩니다.

집계 테이블은 일/월 단위라 행 수가 적고 UPSERT 빈도도 낮아 `record_key` 외래키를 유지했습니다.
