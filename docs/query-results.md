...
제공된 입력 데이터 4개 파일을 **실제 API 로 수집한 뒤 조회 API 를 호출한 결과**다.
아래 표와 JSON 은 손으로 작성한 것이 아니라 응답을 그대로 옮긴 것이다.
...
`INPUT_DATA1.json` 을 한 번 더 전송한 결과다. 재전송은 오류가 아니므로 200 으로 응답하고,
저장 건수와 중복 건수를 나눠 알려준다. 원본 데이터도 집계값도 늘지 않는다.
...
> 애플 HealthKit 은 칼로리를 제공하지 않아 항상 0 이 들어온다.
> `caloriesSupported: false` 로 구분해 내려주므로, 클라이언트가 "활동이 없었다" 로 잘못 표시하지 않는다.

> 활동일수가 12월에 15~16일인 것은 입력 데이터가 12월 중순까지만 존재하기 때문이다.
...
측정 데이터가 없는 날은 행이 생략된다. 응답에 조회 조건이 함께 담기므로,
빈 결과가 "활동이 없던 날"인지 "조회 범위 밖"인지 구분할 수 있다.
...
일별 집계 126행 전부를 원본 테이블과 대조했을 때 불일치가 없다.
...
### 3. 소수 걸음수를 합산 후 한 번만 반올림한다

애플 데이터의 걸음수는 소수다. 구간별로 반올림한 뒤 더하면 원본 합계와 어긋난다.
...
차이는 크지 않지만 조회 결과가 원본과 정확히 일치해야 하므로 합산 후 1회 반올림을 택했다.
